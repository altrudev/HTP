...
fn snapshot() -> htp_reference::LiveWitnessSnapshot {
    ...
    assembler.ingest(ConversationEvent {
    ...
    }).unwrap().expect("response closes turn")
}
...
fn public_content_binding_rejects_mismatched_stream_root() {
    ...
    assert!(!report.valid);
    assert!(report.issues.iter().any(|issue| issue.contains("stream root does not match envelope")));
}

#[test]
fn valid_envelope_cannot_be_reused_for_another_transaction() {
    let first = snapshot();
    let envelope = SignedWitnessEnvelope::sign(&first, PublicationProfile::Private, "test-key", SECRET).unwrap();
